...
public class Sen2VMDirectTest
{
    ...
    public void testDirectLocRawShifted()
    {
        ...
        try
        {
            String nameTest = "testDirectLocRawShifted";
            String outputDir = Config.createTestDir(Config.TDS.TDS2, nameTest, "direct");
            String config = Config.configRawShifts(configTmpDirectTDS2, outputDir, stepBand10m, "direct", false, 48, 32, 16);
            String param = Config.changeParams(paramTmp, detectors, bands, outputDir);
            String[] args = {"-c", config, "-p", param};
            Sen2VM.main(args);
            Utils.verifyDirectLoc(config, refDir + "/" + nameTest);
            ...
            Utils.verifyDirectLoc(config, refDir + "/" + "testDirectLocRawShiftedZero",0.028); 
            // We are expecting a failure
            // LOGGER.warning("Comparison with not shifted data, failure expected");
        } catch (Sen2VMException e) {
            LOGGER.warning(e.getMessage());
            e.printStackTrace();
            assert(false);
    ...
    }//TODO Review threshold + add comparison that results are different (if possible close to the expected shift)
}