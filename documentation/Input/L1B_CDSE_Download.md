...
This guide explains how to download **Sentinel-2 Level-1B** (S2, L1B) data from the [Copernicus Data Space Browser](https://browser.dataspace.copernicus.eu/) at format EUP.COMPACT_SAFE, compatible with Sen2VM. Sentinel-2 L1B products are raw data requiring expert knowledge for orthorectification and processing. Access to these products is restricted to **expert users** only:refs[3-2,65].
...
Go to the **Search** tab (upper left) and select **"Sentinel-2"** from the mission list.
...
In the product details, select the **"Sentinel2\:L1B"** workflow (in `WORKFLOWS`, below `ATTRIBUTES`)
